CREATE OR REPLACE VIEW view_cpu_values_per_minute AS
    ...
    FROM cpu_values
    GROUP BY username, TO_CHAR(tstp, 'YYYY/MM/DD HH24:MI')
    ORDER BY tstp DESC;
...
CREATE OR REPLACE VIEW view_cpu_values_per_hour AS
    ...
    FROM cpu_values
    GROUP BY username, TO_CHAR(tstp, 'YYYY/MM/DD HH24')
    ORDER BY tstp DESC;
...
CREATE OR REPLACE VIEW view_cpu_values_per_day AS
    ...
    FROM cpu_values
    GROUP BY username, TO_CHAR(tstp, 'YYYY/MM/DD')
    ORDER BY tstp DESC;
...
CREATE OR REPLACE VIEW view_cpu_values_per_month AS
    ...
    FROM cpu_values
    GROUP BY username, TO_CHAR(tstp, 'YYYY/MM')
    ORDER BY tstp DESC;
...
CREATE OR REPLACE VIEW view_cpu_values_per_year AS
    ...
    FROM cpu_values
    GROUP BY username, TO_CHAR(tstp, 'YYYY')
    ORDER BY tstp DESC;